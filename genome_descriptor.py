...
GSCtool is the first genomic descriptor that characterizes genome
for applying machine learning in genomics. GSCtool counts the number 
of single nucleotide polymorphisms (SNPs) located in a gene region 
based on genomic general feature format (GFF3) and variant call 
format (VCF).
...
def vcf_info_parse(line):
    ...
    sample_genotype = gentoye2genobin(
...
def main():
    ...
    parser.add_argument('--gzvcf',
                        help='The gzvcf file of individual or population',
                        required=True)
    parser.add_argument(
        '--gff3',
        help='The gff3 file which corresponds to the spieces of the gzvcf',
        required=True)
    parser.add_argument(
        '--outfile',
        help=
        'The name of output file with csv format, which records the features of genome',
        required=True)
        ...
        for gff_records in gffs:
            ...
            if biotype and biotype[0] == 'gene':
                ...
                if anchor and site_location(position, star, end):
                    descriptor[geneid] = descriptor[
                        geneid] + genotypes  #利用array的广播机制记录vcf的杂合性
                    anchor = False
                for line in vcf:
    ...
    pd.DataFrame(descriptor,
                 index=header_info[0].split('\t')[9:]).to_csv(args.outfile)
